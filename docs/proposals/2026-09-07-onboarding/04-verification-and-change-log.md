...
## 5. Outreach link made general (9 September)

The campaign link is no longer tied to one search. `outbound_campaigns.kind` is `general` (default) or `search`, and `job_id` is nullable. A general link, `refery.xyz/go/join`, shows the two live searches that need people most, one engineering and one GTM, chosen by rule at render time in `lib/outreach/featured.ts`: the search pinned for that function (`partner_roles.outreach_pinned_at`), else the highest priority, then the fewest live submissions, then the one open longest; only searches still sourcing count. Each card shows the title with the client's name stripped, the stage band, city and on-site or remote, "Hiring now" when urgent, the payout line, and the one-clause summary only when `preview_approved` is true. `/admin/campaigns` shows the current pair with the client's name for Lily, a kind switch on the new-link form, and an "Anonymised previews" list per live search (save, approve, withdraw, pin) behind `PATCH /api/admin/campaigns`, which refuses words that name the client. Drafts were written for all eleven live searches on 9 September, none approved. The DM and the cold email carry the one general link; a one-search link is still possible for an audience built around a single search.
