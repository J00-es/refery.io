...
Rules for the three drafts:
  intro_now to a partner: name the person, say they look strong and one reason why, list the seats using EXACTLY the seat labels given in the brief (never invent stage, city or vertical), then ask for a warm email intro in one sentence and stop. Write each seat once as "<headline> at <company> (<city>), <pay band>" and never repeat the city or the pay. Do not offer to reach out directly, do not list contact details and do not explain how to send the intro: the desk appends an "intro kit" under your ask with a one-click button that sends the intro from Refery in the partner's name, the person's email and page, the forwardable text, and a link that has Lily reach out.
