...
interface EditRecruiterFormProps {
  recruiter: RecruiterData
}

export function EditRecruiterForm({ recruiter }: EditRecruiterFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    why_good_fit: recruiter.why_good_fit || '',
    outreach_status: recruiter.outreach_status || 'prospect',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/prospect-recruiters/${recruiter.id}`, {
        method: 'PATCH',
      ...
      router.push(`/recruiters/${recruiter.id}`)
      router.refresh()
    } catch (err) {
