...
import type { JobInternalNote } from '@/lib/types'

interface JobInternalNotesProps {
  jobId: string
}
...
const noteTypeLabels: Record<string, { label: string; color: string }> = {
  ...
  candidate: { label: 'Candidate', color: 'bg-green-100 text-green-700' },
  change: { label: 'Change', color: 'bg-amber-100 text-amber-700' },
  strategy: { label: 'Strategy', color: 'bg-purple-100 text-purple-700' },
}

export function JobInternalNotes({ jobId }: JobInternalNotesProps) {
  const [notes, setNotes] = useState<JobInternalNote[]>([])
    ...
    <Card>
      <CardHeader>
        ...
        <p className="text-xs text-muted-foreground">
          Only visible to recruiters and admins
        </p>
      </CardHeader>
