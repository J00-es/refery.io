...
export function StageOverviewCard({
  ...
  return (
    <Link href={href}>
      <div className="bg-white border border-[rgba(22,22,19,0.10)] rounded-[10px] overflow-hidden cursor-pointer transition-all hover:border-[rgba(22,22,19,0.20)] hover:-translate-y-0.5">
        <div className="h-[3px]" style={{ backgroundColor: accentColor }} />
        <div className="px-3 sm:px-[18px] py-3 sm:py-4">
          <p className="text-[10px] sm:text-xs text-[rgba(22,22,19,0.64)] font-medium tracking-wide mb-1 sm:mb-2">
            {stageName}
          </p>
          <div className="flex items-baseline justify-between gap-2">
            <span className="text-[26px] sm:text-[32px] font-semibold leading-none tracking-tight text-[#161613]">
              {count}
            </span>
            {weeklyDelta !== 0 && (
              <span className={cn(
                'text-xs font-medium',
                weeklyDelta > 0 ? 'text-[#1F3A2F]' : 'text-[#B23B3B]'
              )}>
                {weeklyDelta > 0 ? '+' : ''}{weeklyDelta} this wk
              </span>
            )}
            {weeklyDelta === 0 && (
              <span className="text-xs text-[rgba(22,22,19,0.40)]">
                no change
              </span>
            )}
          </div>
          {subText && (
            <p className="text-[11px] text-[rgba(22,22,19,0.40)] mt-1">{subText}</p>
          )}
          {criticalCount > 0 && (
